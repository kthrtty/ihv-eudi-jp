...
| Worker | URL | エントリ | 役割 |
|---|---|---|---|
| `issuer` | `https://issuer.kthrtty.workers.dev` | `src/worker-issuer.mjs` | OID4VCI 発行 + デモ検証コンソール |
| `verifier` | `https://verifier.kthrtty.workers.dev` | `src/worker-verifier.mjs` | OID4VP + DC API ページ |
| `web-wallet` | `https://web-wallet.kthrtty.workers.dev` | `src/worker-wallet.mjs` | ブラウザウォレット |
...
(`*.kthrtty.workers.dev`) で DC API 実機テストを行う際は、証明書を本番 SAN で
