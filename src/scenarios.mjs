...
export const SCENARIOS = {
  ...
  'disaster-aid': {
    ...
    story: '地震で住まいに被害を受けた契約者が、保険金をオンラインで請求します。本人確認ののち、市区町村が発行した罹災証明書を提示します。被害程度（全壊・半壊など）が保険金の査定に使われます。なお身分証の住所と罹災住家の所在地は一致しないことがあります（賃貸・二世帯・被災後の転居など）。統一様式がこの2つを別項目にしているのはそのためで、不一致は受理を妨げず注意として表示します。',
    ...
    checks(pid, eaa, r2) {
      // **住所の不一致は「受理しない理由」にならない**（2026-08-31 修正）。
      // 統一様式（府政防第737号）が**世帯主住所と被災住家の所在地を別項目**にしているのは、
      // **一致しないことが普通にあるから**——賃貸・二世帯・単身赴任・空き家、そして
      // 罹災後に住民票を移した場合。被災した家に住民票が無いことも珍しくない。
      // 一致を受理条件にすると、**正当な請求を機械的に落とす**（実際に通過できなかった）。
      //
      // 実務でも保険会社は物件を証券の対象物件と突き合わせるので、身分証の住所と
      // 一致するかは判断材料の1つにすぎない。**注意喚起（`warn`）に留める**。
      const addrSame = !!(pid && eaa)
        && strip(cl(pid, 'residence_address')) === strip(cl(eaa, 'address'));
      return [
        { ok: sameName(pid, eaa), label: '身分証と罹災証明書の氏名が一致' },
        addrSame
          ? { ok: true, label: '住所が罹災住家と一致（保険の対象物件の確認）' }
          : { ok: true, warn: true,
