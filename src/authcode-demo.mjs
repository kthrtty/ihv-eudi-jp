...
const CSS = `
  ...
  *{box-sizing:border-box}
  ...
  /* 全画面共通ディスクレーマー（shell/appShell が body 末尾に挿入） */
  .demofoot{max-width:1104px;margin:30px auto 22px;padding:0 22px;text-align:center;font-size:11px;line-height:1.7;color:var(--muted,#5B6B82)}
`;
...
// 全画面共通ディスクレーマー（issuer/verifier/wallet）
const DEMO_FOOT = '<footer class="demofoot">表示される組織名・クレデンシャル名は全てデモ目的の架空のものです</footer>';
...
<body class="role-${role}">${SENTINEL}<header class="top ${role}"><span class="tag"></span><div><b>${esc(brand)}</b><small>${esc(sub)}</small></div>${right}</header><div class="${cls}">${body}</div>${DEMO_FOOT}${dev ? devWidgetHtml() : ''}${STICKY_JS}</body></html>`;
};
...
export function renderLogin(users, next = '/', { note = null } = {}) {
  ...
  return `<!doctype html><html lang="ja"><head>
    ...
    <div style="text-align:center;max-width:700px;padding:0 24px">
      ...
      <footer style="margin-top:18px;font-size:11px;color:#5B6B82">表示される組織名・クレデンシャル名は全てデモ目的の架空のものです</footer>
    </div>
  </body></html>`;
}
...
export function appShell(title, body, user = null, { width = 'narrow', dev = true } = {}) {
  ...
  return `<!doctype html><html lang="ja"><head>
    ...
    ${SENTINEL}${appHeaderHtml(user, dev)}<div class="${cls}">${body}</div>${DEMO_FOOT}${dev ? devWidgetHtml('', { endpoints: true }) : ''}${STICKY_JS}
  </body></html>`;
}
