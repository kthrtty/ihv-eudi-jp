...
const IP = 8975, VPP = 8976;
const ISSUER = `http://127.0.0.1:${IP}`, VERIF = `http://127.0.0.1:${VPP}`;
let issuerServer, verifierServer;
test.before(() => {
  issuerServer = serve({ fetch: createApp({ credentialIssuer: ISSUER }).fetch, port: IP });
  verifierServer = serve({
    fetch: createVerifierApp({ verifierOrigin: VERIF, walletOrigin: 'http://127.0.0.1:8979', issuerUrl: ISSUER }).fetch,
    port: VPP,
  });
});
test.after(async () => {
...
async function accountPost(sid, userId, extra) {
  const u = await (await fetch(`${ISSUER}/users/${userId}`)).json();
  const base = {
...
test('更新→発行→提示→表示→履歴 E2E: オファー後にアップロードした写真が発行VCに載り、検証結果と履歴に描画される', async () => {
  ...
  const after = await (await fetch(`${ISSUER}/users/u_002`)).json();
...
test('保持と表示: web wallet の表示キャッシュは data URI、詳細ページは <img>、一覧カードにPIIなし', async () => {
  ...
  const wapp = createWalletApp({ walletOrigin: 'http://127.0.0.1:8979', issuerUrl: ISSUER, verifierUrl: VERIF });
  const add = await wapp.request('/add?credential_offer_uri=' + encodeURIComponent(`${ISSUER}/offer/${made.offer_id}`));
  assert.equal(add.status, 200);
  const receipt = await add.text();
...
test('更新の防御と表示: 256KB 超は拒否（マジックバイトが正しくても）、アカウント画面はプレビュー+由来表を描画', async () => {
  ...
  const u = await (await fetch(`${ISSUER}/users/u_004`)).json();
