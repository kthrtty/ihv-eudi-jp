...
名前空欄行は drop=JS無し削除）。
**発行ポータルUI（2026-07-11 刷新）**: `renderVcSelect` は**書類カタログ型**（旧 `typeIcon` の白タイル・2段行=名前は全幅で省略禁止／説明+形式チップ mdoc/SD-JWT）。プレビュー（ウォレットに入る姿）は `walletCardCss` のカードを選択数ぶん **-46px で重ねるスタック**（count↔重なり連動）で、**レスポンシブに提示先を切替**: **PC(≥820px)=常時表示のサイドレール**（`.railcol`・2カラム右・`#railStack`/`#issueRail`/⚙発行オプション集約）／**SP=固定アクションバー**（`.actbar`・選択数はサムネイル右上の `.ab-badge`・テキストは幅460px超のみ）**＋ボトムシート**（`.psheet`・下から出て画面遷移なし・`#pstack`/`#issueSheet`）。発行は3経路（`#issue`/`#issueSheet`/`#issueRail`）とも `doIssue()`。⚙は `#optbtn`/`#optbtnRail` が共通 `#optpanel` をトグル。旧 `.vccard`/`.vcgrid`/`.ibar` は廃止。発行フロー（buildOffer→showResult→#out受け渡し→revealOut）は不変（revealOut は `.actbar` 基準）。回帰 pin: `select.sel{width:280px}`。
