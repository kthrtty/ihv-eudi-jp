...
- **「Annex C 対応」は誇大だった（2026-07-09 判明）**: 本実装の `org-iso-mdoc` は protocol 名のみ Annex C で、
  data は本来の `{deviceRequest(CBOR・readerAuth), encryptionInfo}` でなく **DCQL を運ぶ独自簡略形＝実機非互換**
  （docs/mdoc-handover.md には正しい構造が記載済みなのに実装と未突合）。暗号面（HPKE/EncryptionInfo/
  SessionTranscript `["dcapi",hash]`）のみ準拠。実機 Multipaz で通っているのは `openid4vp-v1-unsigned`+DCQL。
  **教訓: 適合を名乗る面は自己ループでなく外部実装との適合テストで pin。簡略化は名乗りに明示。**
...
テストは `driveAdd()` ヘルパで step を完走させる。テストのポートは並列実行で他ファイルと衝突させない（8975/76 は webwallet 使用）。
