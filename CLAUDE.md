...
- **同じ対象の重複は検出して知らせるだけ**（自動失効はしない）。重複申請の却下は自治体の判断＝
  実運用に落ちるため。`duplicateApprovals()` が同じ人・同じ対象キーの認定済み申請を返し、審査画面が警告する。
  対象キー `targetKey()`＝罹災は災害名＋被災住家 / 離島は交付自治体＋対象離島。
  **表記揺れは吸収する**（NFKC・ハイフン様記号・空白・大小文字）が丸めすぎない
  （カタカナ長音 ー は寄せない。「3丁目1番5号」と「3-1-5」は解けないので目視前提）——
  誤検出は正当な申請を却下させかねないため
