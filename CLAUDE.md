...
名前空欄行は drop=JS無し削除）。
**発行ポータルUI（2026-07-11 刷新）**: `renderVcSelect` は**書類カタログ型**（旧 `typeIcon` の白タイル・2段行=名前は全幅で省略禁止／説明+形式チップ mdoc/SD-JWT）＋**固定アクションバー**（選択数サムネイル重ね+「プレビュー」+「発行」で選択画面から直接発行）＋**ボトムシートプレビュー**（`walletCardCss` のカードを選択数ぶん -46px で重ねる＝count↔重なり連動・画面遷移なし）。⚙オプション（grant/delivery/tx_code/JSONプレビュー）はバー直上の `.optpanel`。旧 `.vccard`/`.vcgrid`/`.ibar` は廃止。発行フロー（buildOffer→showResult→#out受け渡し→revealOut）は不変（revealOut は `.actbar` 基準）。テスト `test/e2e-portal`(手動)+`select.sel{width:280px}` の回帰 pin は維持。
