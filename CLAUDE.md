...
**カード詳細＝同ページ内オーバーレイ（2026-07-12・増分1）**: ホームのカードクリックは `/cred/:id` 遷移でなく、`/cred/:id?embed=1`
...
`credStatus`)/有効期限(`expiry_date` claim)/形式 の4点を常時表示。レイアウトは **PC=2カラム（属性左・履歴/失効右）/モバイル=1カラム**
（`.wd-wrap` grid・760px 境界）。ドラッグ並び替えとは非干渉（capture 段の `preventDefault` と `body.wd-active` ガードで両立）。
残: PCの「左スタック×右リスト連動ホーム」は並び替え再設計を伴うため増分2。回帰=`test/webwallet.test.mjs` の「必ず表示バンド」。
