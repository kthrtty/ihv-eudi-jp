...
- **Status List の事前確保は「発行数を漏らさないための固定長」**（2026-08-16・#30。既定 65536）。守るべき不変条件が3つ:
  ...
  (3) **ビット列は packed で持つ——永続化だけでなくメモリ上も**（2026-08-30 に後者を修正）——0/1 の JSON 配列だと 1ビットが3バイトで、65536×3本＝477KB。
...
- **#45 の迂回路は「アンカーを見ない」だけでは足りない**。suite の VC はヘッダが
...
- **VP を通すのに要る設定は3つで、どれか1つ欠けると全モジュールが落ちる**（2026-08-30・再測で3回踏んだ）。
  ...
  (1) **`vcfg:features` の `verifier_trust_presented_jwk`**（#45 の迂回路）／
...
- **1ビットに1要素の配列で持たない**（2026-08-30・実測）。#30 で「JSON の往復だけで 5ms」を
  理由に packed 永続化へ移したのに、**`restore()` が 0/1 の JS 配列へ展開し直していた**
  ——同じ罠の裏返しを踏んでいた。実測は **64倍**（65,536 で 0.5MB / 2^24 で **127.5MB**。
  packed なら 8KB / 2.0MB）で、**`_loadState()` は毎アクセス呼ばれる**ので展開コストが
  毎回かかる: **65,536×5本の展開だけで 10.78ms** ＝ Workers の CPU 上限（1リクエスト 10ms）を
  単独で使い切る。packed のまま保持したら **restore 10.78ms → 0.066ms / メモリ 2.5MB → 5KB**。
  読み書きは `bitAt` とビット演算（`b[i>>3] |= 1 << (i&7)`）。道具（`packBits`/`bitAt`）は
  最初からあったので、使い方の問題だった。**なお 1ビット1バイトの `Uint8Array` も答えではない**
  （それでも8倍）。回帰=test/status.test.mjs（**バイト境界の隣が汚れないこと**を見る——
  ビット演算の取り違えはそこに出る）
...
- **issue 番号は書く前に `gh issue view <n>` で確かめる**（2026-08-30）。CLAUDE.md が
  `verifier_trust_presented_jwk` の迂回路を **#42** と書いていたが、#42 は
  「Status List: 枠を使い切ったときに次のパーティションへ切り替える」で**全くの別物**だった。
  しかも迂回路の issue は**そもそも存在しなかった**（#45 として起票）。
  誤った番号は**書き足すときに踏襲されて増える**——今回も1箇所の誤りが2箇所になっていた
...
**モデルの使い分けは `harness/model-routing.md`**: **プランニングと品質チェックは Opus / Fable、
単純作業は Sonnet のサブエージェントへ積極的に委譲する**。委譲はコスト削減のためだけでなく、
**手数の多い作業でメインの文脈を食い潰さず、判断に専念させるため**。
委譲するときは (1) 調査済みの事実を渡す (2) **判定させず観測事実だけ書かせる**
(3) 禁止事項を列挙（デプロイ・コミット・本番アクセス・**コードを直さない**）
(4) 報告の形を指定 (5) **期待値を書かない**（先入観を与えない）。
**報告は鵜呑みにしない**——実際に誤報を受けた例が複数ある（詳細は同ファイル）。
