...
- **`scripts/gen-schemas.mjs` は陳腐化しており再実行禁止**（2026-07-10 実害）: `household_members`／PID `age_over_NN` 等は
  schemas/*.json へ直接追加され生成器に未反映。再生成すると消えてテスト13件が落ちる。schemas/ の正は tracked JSON。
  生成器を使うなら先に差分を還元すること
...
短押しは通常の詳細遷移・iOSリンクプレビューは touch-callout:none で抑止・PC2カラム格子では無効）。
