...
**モバイルの詳細は「持ち上げ＋折り重ね」演出**（2026-07-12・`isMobile()`=max-width:899px で分岐）: `#wdMobile` 固定レイヤーに
**ホームカードの複製**を載せ、選択カードを上へ持ち上げ・他カードは下部 `#wdMFold` に**判読不可の 13px 色帯スリーバ（最大4）**で
折り重ね、その下に `#wdMPanel`（フラグメント・券面は非表示＝持ち上げた複製が担う）。**「さらに表示」`data-wd-more`** で
`.expanded`＝折り重ねをフェードアウトし残り属性(`.wd-rest`)・履歴/開発者/削除(`.wd-extra`)を展開。**ホーム/並び替えには一切触れない**
（複製方式）。PC は従来どおりオーバーレイ2カラム（属性右）。PC は DOM 順で全表示（`.wd-more` 非表示）。
